...
export default function Programmers() {
  const [devPeeps] = useState(listOfAwesome);
  ...
  const getNameOfFeatured = () => {
    ...
    const featuredDevObj = devPeeps.find(dev => dev.id === featuredDev);
    return featuredDevObj.name;
  };

  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: featuredDev ? 'gold' : 'royalblue', // 🤔 color turns to gold, when celebrating
  };
      ...
      <div className='programmers'>
        {
          ...
          devPeeps.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => {setFeaturedDev(dev.id)}}>Feature</button>
            </div>
          )
        }
      </div>
